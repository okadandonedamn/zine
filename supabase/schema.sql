-- =============================================================
-- ZINE データベーススキーマ (Supabase / Postgres)
-- Phase 7 で SQL Editor からこのファイルを実行する。
-- 設計思想: すべての活動は feed_items に流れ込む (Timeline First)
-- =============================================================
...
create table feed_items (
  ...
  work_id uuid references works(id) on delete set null, -- 作品紐付き活動の高速フィルタ用
...
-- record_sessions: 1回ごとの活動ログ (Studyplus的な記録の単位)
...
create table thread_replies (
  ...
  user_id uuid references profiles(id) on delete set null, -- 匿名でも本体は保持(モデレーション用)
  number int not null,
  display_name text not null default '名無しの批評家',
  body text not null,
  quote_number int,
  hidden boolean not null default false, -- モデレーターが非表示化
  created_at timestamptz not null default now(),
...
create table notifications (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references profiles(id) on delete cascade,
  kind text not null check (kind in ('like','comment','follow','repost','quote','reply','goal')),
...
-- =============================================================
-- Row Level Security
-- 原則: 公開は全員が読める / 非公開は本人だけ / 書き込みは本人だけ
-- =============================================================
...
create policy "follows self manage" on follows for insert with check (auth.uid() = follower_id);
create policy "follows self delete" on follows for delete using (auth.uid() = follower_id);

-- 作品: 全員が読める / ログインユーザーが登録できる
...
-- feed_items: 公開は全員 / 非公開は本人のみ (followers は将来 follows を参照する条件を追加)
...
create policy "posts self update" on posts for update using (auth.uid() = user_id);
create policy "posts self delete" on posts for delete using (auth.uid() = user_id);
...
create policy "comments self delete" on comments for delete using (auth.uid() = user_id);
...
-- テンプレート: システム提供(owner null)は全員 / 自作は本人のみ
...
-- 掲示板: 読みは全員 / スレ・レスはログインユーザー / 非表示レスはモデレーターと本人のみ
...
create policy "replies readable" on thread_replies for select
...
create policy "replies moderate" on thread_replies for update
...
create policy "reports readable by mods" on reports for select
  using (reporter_id = auth.uid()
         or exists (select 1 from moderators m where m.user_id = auth.uid()));

-- モデレーター一覧: モデレーターのみ閲覧
create policy "moderators readable" on moderators for select
  using (exists (select 1 from moderators m where m.user_id = auth.uid()));
...
declare
  base text;
begin
  -- メールのローカル部からユーザー名を生成(英数と_のみ、最大15文字+ランダム4文字)
